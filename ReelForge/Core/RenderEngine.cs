...
public sealed class RenderEngine
{
    ...
    public RenderEngine(AppPaths paths, Action<JsonObject> report) { this.paths = paths; this.report = report; }
    ...
    public async Task Render(JsonNode project, bool preview)
    {
        ...
        var input = Path.Combine(paths.Media, clips[0]!); var args = $"-y -loop 1 -i \"{input}\" -t 5 -vf \"format=yuv420p,scale=trunc(iw/2)*2:trunc(ih/2)*2\" -r 30 \"{output}\"";
